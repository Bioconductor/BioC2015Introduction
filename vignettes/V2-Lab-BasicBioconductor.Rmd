...
1. How do you download any package from Bioconductor ? More precisely what 
commands do you use? 

2. using the given GRanges , do the following - 
...
a) extract ranges only from chromosome 3 
b) extract the first five ranges from the GRanges.
c) extract the score and gc column of the GRanges 
d) keep only the standard chromosomes (i.e.) from chromosome 1 to 22, x, y,m
e) change the chromosome naming style i.e. this GRanges contains UCSC style 
of chromosome names, change them to NCBI style of chromosome names. 
f) how do you find out the ranges contained in the gaps of this GRanges object?
g) How do you find out the degree of overlap for all the ranges in a GRanges
object ? ( Hint: ?coverage)

3. Find the package in Bioconductor which contains annotation databases 
...
4. Find the package in Bioconductor thats stores the Full genome sequences for 
...
5. In the 'ratGenes' object above, you get only the entrez gene ids, can you get
the gene names for each gene ?

6. Get the annotation databases from NCBI for Homo sapiens 
